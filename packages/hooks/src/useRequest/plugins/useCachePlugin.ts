...
const useCachePlugin: UseRequestPlugin<unknown, unknown[]> = (
  ...
  watchEffect(() => {
    ...
    if (cacheData && Object.hasOwnProperty.call(cacheData, 'data')) {
      fetchInstance.getState().data = cacheData.data;
      fetchInstance.getState().params = cacheData.params;
      if (staleTime === -1 || new Date().getTime() - cacheData.time <= staleTime) {
        fetchInstance.getState().loading = false;
      }
    }
  ...
  return {
    ...
    onMutate: data => {
      if (cacheKey) {
        ...
        _setCache(cacheKey, {
          data,
          params: fetchInstance.getState().params,
          time: new Date().getTime(),
        });
        // 触发器更新
        unSubscribeRef.value = subscribe(cacheKey, d => {
